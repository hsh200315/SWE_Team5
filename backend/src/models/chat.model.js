...
module.exports = {
    ...
    addchat: async (data) => {
        const {roomId, username, message, isFromAI, mapImage} = data;
        try {
            await run('TRANSACTION BEGIN');
            const result = await run('INSERT INTO Chat(room_id, sender_id, message, is_from_AI, map_image)',
                [roomId, username, message, isFromAI, mapImage]);
